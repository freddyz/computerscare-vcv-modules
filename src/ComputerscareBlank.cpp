...
struct ComputerscareBlank : ComputerscareMenuParamModule {
  ...
  */
  ...
  enum ClockModes { CLOCK_MODE_SYNC, CLOCK_MODE_SCAN, CLOCK_MODE_FRAME };
  ...
  ComputerscareBlank() {
    ...
    nextFileDescriptions.push_back(
        "Load Next (Alphabetical) File in Directory");
    nextFileDescriptions.push_back(
        "Load Previous (Alphabetical) File in Directory");
    nextFileDescriptions.push_back("Load Random File from Directory");

    config(NUM_PARAMS, NUM_INPUTS, NUM_OUTPUTS, NUM_LIGHTS);

    configMenuParam(ANIMATION_SPEED, -1.f, 1.f, 0.f, "Animation Speed", 2, "x",
                    20.f);
    ...
    configMenuParam(ANIMATION_MODE, 0.f, "Animation Mode",
                    animationModeDescriptions);
    configMenuParam(NEXT_FILE_BEHAVIOR, 0.f,
                    "Next File Trigger / Button Behavior",
  ...
  void process(const ProcessArgs& args) override {
    ...
    if (ready && leftExpander.module &&
        leftExpander.module->model == modelComputerscareBlankExpander) {
      expanderConnected = true;
      // me
      float* messageToSendToExpander =
    ...
    if (expanderConnected && clockConnected &&
        (clockMode == CLOCK_MODE_FRAME)) {
    ...
    if (numFrames > 1) {
      ...
      if ((clockTriggered &&
           (clockConnected && clockMode == CLOCK_MODE_SYNC)) ||
          numFrames > 1) {
        if (currentFrame == 0) {
          // checkAndPerformEndAction();
        }
  ...
  void setSyncTime(float syncDuration) {
    ...
    if (params[ANIMATION_MODE].getValue() == ANIMATION_PINGPONG) {
      if (numFrames > 1) {
        float totalDurationForCurrentZeroOffset =
            gifDurationsForPingPong[mapBlankFrameOffset(zeroOffset, numFrames)];
        if (constantFrameDelay) {
          speedFactor = (2 * numFrames - 2) * defaultFrameDelayCentiseconds /
                        syncDuration / 100;
        } else {
      ...
      if (constantFrameDelay) {
        speedFactor =
            numFrames * defaultFrameDelayCentiseconds / syncDuration / 100;
      } else {
  ...
  void setFrameShuffle() {
    ...
    std::shuffle(std::begin(shuffledFrames), std::end(shuffledFrames),
  ...
  void setRegularFrameOrders() {
    ...
    framesPingpong.insert(framesPingpong.end(), framesReverse.begin() + 1,
                          framesReverse.end() - 1);
  }
  ...
  void setTotalGifDurationIfInPingPongMode(std::vector<float> durs) {
    /* the gif has a different total pingpong duration depending on where the
       reset frame is eg a gif with frame durations: 1) 100 2) 5 3) 5 4) 5

            if ponging around frame 1 has duration: 100(1)+ 5(2) + 5(3) + 5(4) +
       5(3) + 5(2) = 125

            but if ponging around frame 3: 5(3) + 5(4) + 100(1) + 5(2) + 100(1)
       + 5(4) = 240
  ...
  */
  void tickAnimation() {
    if (numFrames > 1) {
      ...
      if (clockConnected && clockMode == CLOCK_MODE_SYNC && onFinalFrame) {
        // hold up animation back to 1st frame when controlled.  Wait for clock
        // signal
      } else {
        /*if (animationMode == ANIMATION_FORWARD || animationMode ==
        ANIMATION_SHUFFLE) { nextFrame(); } else if (animationMode ==
        ANIMATION_REVERSE)  { prevFrame();
        }
  ...
  void goToFrame(int frameNum) {
    if (numFrames && ready && frameNum != currentFrame) {
      ...
      mappedFrame +=
          mapBlankFrameOffset(zeroOffset, numFrames) + 10 * numFrames;
  ...
  void dataFromJson(json_t* rootJ) override {
    ...
    if (imageFitEnumJ) {
      imageFitEnum = json_integer_value(imageFitEnumJ);
    }

    json_t* invertYJ = json_object_get(rootJ, "invertY");
    if (invertYJ) {
      invertY = json_is_true(invertYJ);
    }
...
struct KeyboardControlChildMenu : MenuItem {
  ...
  Menu* createChildMenu() override {
    ...
    menu->addChild(construct<MenuLabel>(&MenuLabel::text,
                                        "A,S,D,F: Translate image position"));
    menu->addChild(construct<MenuLabel>(&MenuLabel::text, "Z,X: Zoom in/out"));
    menu->addChild(
        construct<MenuLabel>(&MenuLabel::text, "J,L: Previous / next frame"));
    menu->addChild(
        construct<MenuLabel>(&MenuLabel::text, "K: Go to first frame"));

    menu->addChild(
        construct<MenuLabel>(&MenuLabel::text, "I: Go to random frame"));

    menu->addChild(construct<MenuLabel>(
        &MenuLabel::text,
        "[  (left square bracket): Load previous image from same directory"));
    menu->addChild(construct<MenuLabel>(
        &MenuLabel::text,
        "]  (right square bracket): Load next image from same directory"));
    menu->addChild(construct<MenuLabel>(
        &MenuLabel::text, "O: Load random image from same directory"));
    ...
    if (blankModule && blankModule->loadedJSON) {
      ...
      if (path != modulePath) {
        ...
        if (numImageFrames > 1) {
          ...
          blankModule->setTotalGifDurationIfInPingPongMode(
              gifBuddy.getPingPongGifDuration());
...
struct GiantFrameDisplay : TransparentWidget {
  ...
  GiantFrameDisplay() {
    ...
    description->value =
        "Frame Zero, for EOC output, reset input, and sync mode";
  ...
  void step() {
    if (module) {
      ...
      frameDisplay->value =
          string::f("%i / %i", module->scrubFrame + 1, module->numFrames);
    } else {
      visible = false;
    }
...
struct ComputerscareBlankWidget : ModuleWidget {
  ...
  void appendContextMenu(Menu* menu) override {
    ...
    menu->addChild(
        construct<MenuLabel>(&MenuLabel::text, "Current Image Path:"));
  ...
  void onHoverKey(const event::HoverKey& e) override {
    ...
    if ((e.mods & RACK_MOD_MASK) == (RACK_MOD_CTRL | GLFW_MOD_SHIFT))
            ...
            arrowSpeed /= 4.0;*/
    // duplicate is ctrl-d, ignore keys if mods are pressed so duplication
    // doesnt translate the image
    if (e.action == RACK_HELD && !e.mods) {
      if (e.keyName == "a") {
        ...
        blankModule->yOffset -=
            (blankModule->invertY ? dPosition : -dPosition) /
            blankModule->zoomY;
        e.consume(this);
      } else if (e.keyName == "d") {
        blankModule->xOffset -= dPosition / blankModule->zoomX;
...
Model* modelComputerscareBlank =
    createModel<ComputerscareBlank, ComputerscareBlankWidget>(
        "computerscare-blank");