...
const std::vector<NVGcolor> outlineColorMap = {COLOR_COMPUTERSCARE_RED,
                                               COLOR_COMPUTERSCARE_YELLOW,
                                               COLOR_COMPUTERSCARE_BLUE};
...
struct ComputerscareILoveCookies : Module {
  ...
  ComputerscareILoveCookies() {
    ...
    for (int k = 0; k < numKnobs; k++) {
      configParam(KNOB_PARAM + k, 0.f, 10.f, 0.0f,
                  string::f("knob %c", knoblookup[k]));
  ...
  void setNextAbsoluteSequence(int index) {
    newABSQueue[index] =
        AbsoluteSequence(upcomingFormula[index], knobandinputlookup);
  ...
  void resetOneOfThem(int i) { newABS[i].readHead = -1; }
  ...
  float mapKnobValue(float rawValue, int rowIndex) {
    ...
    switch (mapEnum) {
      case 0:
        ...
        break;  // 0..10
      case 1:
        mappedValue = mapValue(rawValue, 0.f, 0.5);
        break;  // 0..5
      case 2:
        mappedValue = mapValue(rawValue, 0.f, 0.2);
        break;  // 0..2
      case 3:
        mappedValue = mapValue(rawValue, 0.f, 0.1);
        break;  // 0..1

      case 4:
        mappedValue = mapValue(rawValue, -5, 2.f);
        break;  //-10..10
      case 5:
        mappedValue = mapValue(rawValue, -5, 1.f);
        break;  //-5..5

      case 6:
        mappedValue = mapValue(rawValue, -5, 0.4);
        break;  //-2..2

      case 7:
        mappedValue = mapValue(rawValue, -5, 0.2);
        break;  //-1..1
    }
    return mappedValue;
  }
  ...
  void checkTextField(int channel) {
    ...
    if (textFieldValue != currentFormula[channel] &&
      ...
      AbsoluteSequence pendingSequence =
          AbsoluteSequence(textFieldValue, knobandinputlookup);
...
void ComputerscareILoveCookies::process(const ProcessArgs& args) {
  ...
  if (checkCounter > checkCounterLimit) {
    if (!jsonLoaded) {
      for (int i = 0; i < numFields; i++) {
        // currentTextFieldValue[i] = i < numFields - 1 ? std::to_string(i + 1)
        // : "abcd";
        manualSet[i] = true;
      }
      for (int i = 0; i < numFields; i++) {
  ...
  for (int i = 0; i < numFields; i++) {
    ...
    if (true) {
      if (inputs[CLOCK_INPUT + i].isConnected()) {
        ...
        if ((inputs[GLOBAL_CLOCK_INPUT].isConnected() &&
             globalTriggerClocked) ||
    ...
    if (outputConnected) {
      if (activeKnobIndex[i] < 0) {
        ...
        outputs[TRG_OUTPUT + i].setChannels(
            inputs[SIGNAL_INPUT + activeKnobIndex[i] - 26].getChannels());
        ...
        outputs[TRG_OUTPUT + i].setVoltage(
            newABS[i].exactFloats[activeKnobIndex[i] - 52]);
    ...
    if (inputs[CLOCK_INPUT + i].isConnected()) {
      outputs[FIRST_STEP_OUTPUT + i].setVoltage(
          (currentTriggerIsHigh && atFirstStep) ? 10.f : 0.0f);
    } else {
      outputs[FIRST_STEP_OUTPUT + i].setVoltage(
          (globalGateIn && atFirstStep) ? 10.f : 0.0f);
    }
  }
}
...
struct WiggleKnobsMenuItem : MenuItem {
  ...
  void onAction(const event::Action& e) override { cookies->wiggleKnobs(); }
};
  ...
  ComputerscareILoveCookiesWidget(ComputerscareILoveCookies* module) {
    ...
    for (int i = 0; i < numKnobRows; i++) {
      for (int j = 0; j < numKnobColumns; j++) {
        ...
        ParamWidget* knob = createParam<SmoothKnob>(
            mm2px(Vec(knobPosX, knobPosY)), module,
            ComputerscareILoveCookies::KNOB_PARAM + index);
      ...
      for (int m = 0; m < numInputColumns; m++) {
        ...
        if (m % 2) {
          addInput(createInput<InPort>(
              mm2px(Vec(inputPosX, inputPosY)), module,
              ComputerscareILoveCookies::SIGNAL_INPUT + inputindex));
        } else {
    ...
    // global clock input
    addInput(
        createInput<InPort>(mm2px(Vec(2 + xStart, 0)), module,
                            ComputerscareILoveCookies::GLOBAL_CLOCK_INPUT));

    // global reset input
    addInput(
        createInput<InPort>(mm2px(Vec(12 + xStart, 0)), module,
                            ComputerscareILoveCookies::GLOBAL_RESET_INPUT));
    addParam(createParam<ComputerscareResetButton>(
        mm2px(Vec(12 + xStart, 9)), module,
        ComputerscareILoveCookies::MANUAL_RESET_PARAM));
    addParam(createParam<ComputerscareClockButton>(
        mm2px(Vec(2 + xStart, 9)), module,
        ComputerscareILoveCookies::MANUAL_CLOCK_PARAM));

    for (int i = 0; i < numFields; i++) {
      // first-step output
      addOutput(createOutput<OutPort>(
          mm2px(Vec(42 + xStart, verticalStart + verticalSpacing * i - 11)),
          module, ComputerscareILoveCookies::FIRST_STEP_OUTPUT + i));

      // individual output
      addOutput(createOutput<OutPort>(
          mm2px(Vec(54 + xStart, verticalStart + verticalSpacing * i - 11)),
          module, ComputerscareILoveCookies::TRG_OUTPUT + i));

      // individual clock input
      addInput(createInput<InPort>(
          mm2px(Vec(2 + xStart, verticalStart + verticalSpacing * i - 10)),
          module, ComputerscareILoveCookies::CLOCK_INPUT + i));

      // individual reset input
      addInput(createInput<InPort>(
          mm2px(Vec(12 + xStart, verticalStart + verticalSpacing * i - 10)),
          module, ComputerscareILoveCookies::RESET_INPUT + i));

      // sequence input field
      textField = new CookiesTF2(i);
      textField->box.pos =
          mm2px(Vec(1 + xStart, verticalStart + verticalSpacing * i));
      ...
      cookiesSmallDisplay->box.pos =
          mm2px(Vec(21 + xStart, verticalStart - 9.2 + verticalSpacing * i));
      ...
      currentWorkingStepDisplay->box.pos =
          mm2px(Vec(11 + xStart, verticalStart - 7.0 + verticalSpacing * i));
      ...
      addParam(createParam<ComputerscareInvisibleButton>(
          mm2px(Vec(21 + xStart, verticalStart - 9.9 + verticalSpacing * i)),
          module, ComputerscareILoveCookies::INDIVIDUAL_RESET_PARAM + i));
    }
    cookies = module;
  }
...
void ComputerscareILoveCookiesWidget::appendContextMenu(Menu* menu) {
  ...
  RandomizeTextFieldsMenuItem* randomizeTextFieldsMenuItem =
  ...
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, "  0v ... +10v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 0));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, "  0v ...  +5v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 1));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, "  0v ...  +2v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 2));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, "  0v ...  +1v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 3));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, "-10v ... +10v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 4));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, " -5v ...  +5v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 5));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, " -2v ...  +2v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 6));
  menu->addChild(construct<CookiesKnobRangeItem>(
      &MenuItem::text, " -1v ...  +1v", &CookiesKnobRangeItem::cookies,
      cookiesModule, &CookiesKnobRangeItem::knobRangeEnum, 7));
};

Model* modelComputerscareILoveCookies =
    createModel<ComputerscareILoveCookies, ComputerscareILoveCookiesWidget>(
