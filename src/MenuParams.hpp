...
struct MenuParam : MenuEntry {
  ...
  MenuParam(ParamQuantity* param, int type) {
    if (type == 0) {
      ...
      // johnLabel->box.pos = Vec(button->box.size.x + controlRightMargin * 2,
      // 0);
      ...
      addChild(pWidget);
      johnLabel = construct<MenuLabel>(&MenuLabel::text, param->getLabel());
      johnLabel->box.pos = Vec((type == 2 ? slider->box.size.x :
      pWidget->box.size.x) + controlRightMargin * 2, 0); addChild(johnLabel);*/
    } else if (type == 2) {
...
struct ComputerscareMenuParamModule : ComputerscarePolyModule {
  ...
  void configMenuParam(int paramId, float minValue, float maxValue,
    ...
    configParam(paramId, minValue, maxValue, defaultValue, label, unit,
  ...
  void configMenuParam(int paramId, float defaultValue, std::string label = "",
                       std::vector<std::string> options = {}) {
    int size = (int)options.size();
    configParam<MultiselectParamQuantity>(paramId, 0, size - 1, defaultValue,
                                          label);
    pq = paramQuantities[paramId];
  ...
  std::vector<ParamAndType*> getParamList() { return paramList; }
...
struct MultiselectParamQuantity : ParamQuantity {
  std::string getDisplayValueString() override {
    ComputerscareMenuParamModule* menuParamModule =
        reinterpret_cast<ComputerscareMenuParamModule*>(this->module);
...
struct ParamSettingItem : MenuItem {
  ...
  void onAction(const event::Action& e) override { myParam->setValue(myVal); }
  void step() override {
