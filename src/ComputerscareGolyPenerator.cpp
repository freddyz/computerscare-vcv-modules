...
/*
        ...
        knob3: param 1
*/
const std::string GolyPeneratorAvailableAlgorithmsArr[5] = {
...
struct ComputerscareGolyPenerator : ComputerscareMenuParamModule {
  ...
  void updateCurrents() {
    std::vector<float> golyParams = {
        params[IN_OFFSET].getValue(), params[IN_SCALE].getValue(),
        params[OUT_SCALE].getValue(), params[OUT_OFFSET].getValue()};
    goly.invoke((int)std::floor(params[ALGORITHM].getValue()), golyParams,
                params[POLY_CHANNELS].getValue());
  }
  void setAlgorithm(int algoVal) { params[ALGORITHM].setValue(algoVal); }
...
struct setAlgoItem : MenuItem {
  ...
  void step() override {
    rightText = CHECKMARK(
        penerator->params[ComputerscareGolyPenerator::ALGORITHM].getValue() ==
        mySetVal);
    MenuItem::step();
  }
};
...
struct AlgorithmChildMenu : MenuItem {
  ...
  Menu* createChildMenu() override {
    Menu* menu = new Menu;
    menu->addChild(
        construct<MenuLabel>(&MenuLabel::text, "Select an Algorithm... NOW"));
...
struct ComputerscareGolyPeneratorWidget : ModuleWidget {
  ComputerscareGolyPeneratorWidget(ComputerscareGolyPenerator* module) {
    ...
    addLabeledKnob<ScrambleSnapKnob>("Algo", 4, 324, module,
                                     ComputerscareGolyPenerator::ALGORITHM, 0,
                                     0, true);
    addLabeledKnob<SmoothKnob>("center", 28, 80, module,
                               ComputerscareGolyPenerator::IN_OFFSET, 0, 0);
    addLabeledKnob<SmallKnob>("spread", 5, 86, module,
                              ComputerscareGolyPenerator::IN_SCALE, 0, 0);
    addLabeledKnob<SmallKnob>("scale", 33, 290, module,
                              ComputerscareGolyPenerator::OUT_SCALE, 0, 0);
    addLabeledKnob<SmoothKnob>("offset", 2, 284, module,
                               ComputerscareGolyPenerator::OUT_OFFSET, 0, 0);

    channelWidget = new PolyOutputChannelsWidget(
        Vec(28, 309), module, ComputerscareGolyPenerator::POLY_CHANNELS);
    addChild(channelWidget);

    addOutput(createOutput<InPort>(Vec(28, 329), module,
                                   ComputerscareGolyPenerator::POLY_OUTPUT));
  }
  ...
  void addLabeledKnob(std::string label, int x, int y,
                      ComputerscareGolyPenerator* module, int paramIndex,
                      float labelDx, float labelDy, bool snap = false) {
...
Model* modelComputerscareGolyPenerator =
    createModel<ComputerscareGolyPenerator, ComputerscareGolyPeneratorWidget>(
