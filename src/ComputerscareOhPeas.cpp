...
struct ComputerscareOhPeas : Module {
  ...
  void setQuant() {
    this->quant = Quantizer(this->currentFormula.c_str(), this->numDivisions,
                            this->globalTranspose);
  }
  void checkForChange() {
...
void ComputerscareOhPeas::process(const ProcessArgs& args) {
  ...
  int iTranspose =
      floor(numDivisionsKnobValue * params[GLOBAL_TRANSPOSE].getValue());
  ...
  for (int i = 0; i < numChannels; i++) {
    if (outputs[SCALED_OUTPUT + i].isConnected() ||
        outputs[QUANTIZED_OUTPUT + i].isConnected()) {
...
struct SetQuantizationModeMenuItem : MenuItem {
  ...
  void doAction() { peas->evenQuantizeMode = mode; }
  void step() override {
...
struct PeasSmallDisplay : SmallLetterDisplay {
  ...
  void draw(const DrawArgs& args) {
    ...
    if (module) {
      if (type == 0) {
        std::string transposeString = (module->globalTranspose > 0 ? "+" : "") +
                                      std::to_string(module->globalTranspose);
...
void quantizationModeMenuItemAdd(ComputerscareOhPeas* peas, Menu* menu,
                                 bool evenMode, std::string label) {
  SetQuantizationModeMenuItem* menuItem =
      new SetQuantizationModeMenuItem(evenMode);
  menuItem->text = label;
  menuItem->peas = peas;
...
struct ComputerscareOhPeasWidget : ModuleWidget {
  ...
  ComputerscareOhPeasWidget(ComputerscareOhPeas* module) {
    ...
    // "res/ComputerscareOhPeasPanel.svg")));
    box.size = Vec(9 * 15, 380);
    {
      ComputerscareSVGPanel* panel = new ComputerscareSVGPanel();
      panel->box.size = box.size;
      panel->setBackground(APP->window->loadSvg(
          asset::plugin(pluginInstance, "res/ComputerscareOhPeasPanel.svg")));
    ...
    double xx;
    double yy = 18;
    addParam(createParam<MediumSnapKnob>(mm2px(Vec(11, yy - 2)), module,
                                         ComputerscareOhPeas::NUM_DIVISIONS));

    addParam(createParam<SmoothKnob>(mm2px(Vec(21, yy - 2)), module,
                                     ComputerscareOhPeas::GLOBAL_TRANSPOSE));
    ...
    for (int i = 0; i < numChannels; i++) {
      ...
      addInput(createInput<InPort>(mm2px(Vec(xx, y - 0.8)), module,
                                   ComputerscareOhPeas::CHANNEL_INPUT + i));

      addParam(createParam<SmallKnob>(mm2px(Vec(xx + 2, y + 34)), module,
                                      ComputerscareOhPeas::SCALE_TRIM + i));

      addInput(createInput<InPort>(mm2px(Vec(xx, y + 40)), module,
                                   ComputerscareOhPeas::SCALE_CV + i));

      addParam(createParam<SmoothKnob>(mm2px(Vec(xx, y + 50)), module,
                                       ComputerscareOhPeas::SCALE_VAL + i));

      addParam(createParam<ComputerscareDotKnob>(
          mm2px(Vec(xx + 2, y + 64)), module,
          ComputerscareOhPeas::OFFSET_TRIM + i));

      addInput(createInput<InPort>(mm2px(Vec(xx, y + 70)), module,
                                   ComputerscareOhPeas::OFFSET_CV + i));

      addParam(createParam<SmoothKnob>(mm2px(Vec(xx, y + 80)), module,
                                       ComputerscareOhPeas::OFFSET_VAL + i));

      addOutput(createOutput<OutPort>(mm2px(Vec(xx, y + 93)), module,
                                      ComputerscareOhPeas::SCALED_OUTPUT + i));

      addOutput(
          createOutput<InPort>(mm2px(Vec(xx + 1, y + 108)), module,
                               ComputerscareOhPeas::QUANTIZED_OUTPUT + i));
    }
    peas = module;
  }
...
// Model *modelComputerscareDebug = createModel<ComputerscareDebug,
// ComputerscareDebugWidget>("computerscare-debug");

Model* modelComputerscareOhPeas =
    createModel<ComputerscareOhPeas, ComputerscareOhPeasWidget>(
        "computerscare-ohpeas");
