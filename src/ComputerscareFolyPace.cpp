...
struct FolyPace : Module {
  enum ParamIds { TIME_PARAM, TRIM, OFFSET, SCRAMBLE, NUM_PARAMS };
  ...
  void process(const ProcessArgs& args) override {
    ...
    if (bufferIndex < BUFFER_SIZE) {
      if (++frameIndex > frameCount) {
        ...
        trimVal + offsetVal + 99 + (1071 * cmap[c]) % 19;
    ...
    // Trigger immediately if external but nothing plugged in, or in Lissajous
    // mode
    if (true) {
      trigger();
      return;
  ...
  void dataFromJson(json_t* rootJ) override {
    ...
    if (faceEmitsLightJ) faceEmitsLight = json_boolean_value(faceEmitsLightJ);
  }
};
...
struct FolyPaceDisplay : TransparentWidget {
  ...
  void drawFace(const DrawArgs& args, float A, float B, float C, float D,
    ...
    float fr = 0.04 * sin(H - M) + 0.02 * sin(H / 3 + 2.2) +
               0.02 * sin(L + P + 8.222);  // face rotation
    ...
    float leftEyebrowHeight =
        erly * (1.9 + 0.6 * sin(G) + 0.3 * sin(K - B / 2));
    float rightEyebrowHeight =
        erly * (1.9 + 0.6 * sin(G - 2.2 + N) + 0.2 * sin(L + 33));
    ...
    drawEyes(args, epx, epy, eyeSpacing, erlx, erly, 1, irisRad, pupilRad,
             gazeDir, gazeStrength, irisColor, pupilColor);
    drawEyebrows(args, epx, epy, eyeSpacing, leftEyebrowHeight,
                 rightEyebrowHeight, leftEyebrowAngle, rightEyebrowAngle,
                 eyebrowColor, eyebrowThickness, eyebrowLength);
    ...
    NVGcolor mouthLipColor = nvgHSLA(0.1 * sin(N) - 0.1, 0.6 + 0.3 * sin(M),
                                     0.5 + .4 * sin(I), 0xff);

    nvgGlobalCompositeOperation(args.vg, NVG_ATOP);

    drawMouth(args, mouthX, mouthY, mouthWidth, mouthOpen, mouthSmile,
              mouthSkew, mouthThickness, mouthLipColor);

    nvgResetScissor(args.vg);
    // nvgRestore(args.vg);
  }

  void drawEyebrows(const DrawArgs& args, float x, float y, float eyeSpacing,
                    float leftEyebrowHeight, float rightEyebrowHeight,
                    float leftEyebrowAngle, float rightEyebrowAngle,
                    NVGcolor eyebrowColor, float eyebrowThickness,
                    float eyebrowLength) {
    ...
    nvgMoveTo(args.vg, x - eyeSpacing / 2 - r * cosLeft,
              y - leftEyebrowHeight - r * sinLeft);
    nvgLineTo(args.vg, x - eyeSpacing / 2 + r * cosLeft,
              y - leftEyebrowHeight + r * sinLeft);
    // nvgStroke(args.vg);

    nvgMoveTo(args.vg, x + eyeSpacing / 2 - r * cosRight,
              y - rightEyebrowHeight - r * sinRight);
    nvgLineTo(args.vg, x + eyeSpacing / 2 + r * cosRight,
              y - rightEyebrowHeight + r * sinRight);
    nvgStroke(args.vg);

    nvgClosePath(args.vg);
  }
  void drawHead(const DrawArgs& args, float x, float y, float width,
                float height, NVGcolor color) {
    nvgBeginPath(args.vg);
  ...
  void drawMouth(const DrawArgs& args, float x, float y, float width,
                 float open, float smile, float skew, float thickness,
                 NVGcolor lipColor) {
    nvgBeginPath(args.vg);
    ...
    // top
    nvgBezierTo(args.vg, x - width / 4, y - open * smile, x + width / 4,
                y - open * smile, x + width / 2, y - 10.f * smile);

    // bottom
    nvgBezierTo(args.vg, x + width / 4, y + smile * open, x - width / 4,
                y + smile * open, x - width / 2, y - 20.f * smile);
    nvgClosePath(args.vg);
    nvgGlobalCompositeOperation(args.vg, NVG_ATOP);
  ...
  void drawEyes(const DrawArgs& args, float x, float y, float spacing, float rx,
                float ry, float open, float irisRad, float pupilRad,
                float gazeDir, float gazeStrength, NVGcolor irisColor,
                NVGcolor pupilColor) {
  ...
  void drawNose(const DrawArgs& args, float x, float y, float height,
                float width, float thickness, NVGcolor color) {}

  void draw(const DrawArgs& args) override {
    if (!module) {
      drawFace(args, random::uniform() * 10, random::uniform() * 10,
      ...
      if (!module->faceEmitsLight) {
        drawFace(args, module->bufferX[0][0], module->bufferX[1][0],
                 module->bufferX[2][0], module->bufferX[3][0],
                 module->bufferX[4][0], module->bufferX[5][0],
                 module->bufferX[6][0], module->bufferX[7][0],
                 module->bufferX[8][0], module->bufferX[9][0],
                 module->bufferX[10][0], module->bufferX[11][0],
                 module->bufferX[12][0], module->bufferX[13][0],
                 module->bufferX[14][0], module->bufferX[15][0]);
      }
    }
  }
...
struct FolyPaceWidget : ModuleWidget {
  FolyPaceWidget(FolyPace* module) {
    ...
    box.size = Vec(9 * 15, 380);
    {
      ...
      panel->setBackground(APP->window->loadSvg(
          asset::plugin(pluginInstance, "res/ComputerscareFolyPacePanel.svg")));
      addChild(panel);
    }
    ...
    addInput(createInput<PointingUpPentagonPort>(Vec(1, 353), module,
                                                 FolyPace::X_INPUT));
    ...
    addParam(
        createParam<ScrambleKnob>(Vec(81, 357), module, FolyPace::SCRAMBLE));
  }
  ...
  void appendContextMenu(Menu* menu) override {
    ...
    menu->addChild(
        createBoolPtrMenuItem("Face Emits Light", "", &module->faceEmitsLight));
  }
};

Model* modelComputerscareFolyPace =
    createModel<FolyPace, FolyPaceWidget>("computerscare-foly-pace");
