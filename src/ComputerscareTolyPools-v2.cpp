...
struct ComputerscareTolyPoolsV2 : Module {
  ...
  enum InputIds { POLY_INPUT, ROTATE_CV, NUM_CHANNELS_CV, NUM_INPUTS };
  enum OutputIds { POLY_OUTPUT, NUM_CHANNELS_OUTPUT, NUM_OUTPUTS };
  ...
  void process(const ProcessArgs& args) override {
    ...
    if (inputs[NUM_CHANNELS_CV].isConnected()) {
      cvOutputChannels =
          (int)round(inputs[NUM_CHANNELS_CV].getVoltage(0) * 1.6f);
    }
    ...
    numOutputChannelsControlValue =
        math::clamp(cvOutputChannels + numChannelsKnob, 0, 16);
    ...
    outputs[NUM_CHANNELS_OUTPUT].setVoltage(
        mapChannelCountToVoltage(numInputChannels));

    if (rotationModeEnum == 0) {
      rotationBase = inputIsConnected
                         ? numInputChannels
                         : 16;  // so when unconnected, the rotation knob
                                // illustrates the normal range
    ...
    if (inputs[POLY_INPUT].isConnected() &&
        outputs[POLY_OUTPUT].isConnected()) {
      for (int i = 0; i < numOutputChannels; i++) {
        outputs[POLY_OUTPUT].setVoltage(
            inputs[POLY_INPUT].getVoltage((i + finalPositiveRotation) %
                                          rotationBase),
            i);
      }
    } else {
  ...
  json_t* dataToJson() override {
    ...
    json_object_set_new(rootJ, "rotationModeEnum",
                        json_integer(rotationModeEnum));
    return rootJ;
  }
...
struct PoolsSmallDisplayV2 : SmallLetterDisplay {
  ComputerscareTolyPoolsV2* module = nullptr;
  int ch = 0;
  int type = 0;
    ...
    if (module) {
      if (type == 0) {
        ...
        if (module->rotation > 0) {
          ...
          rotationDisplay =
              -1 * ((-1 * module->rotation) % module->rotationBase);
        }
...
struct ComputerscareTolyPoolsWidgetV2 : ModuleWidget {
  ComputerscareTolyPoolsWidgetV2(ComputerscareTolyPoolsV2* module) {
    ...
    addInput(createInput<InPort>(Vec(1, 50), module,
                                 ComputerscareTolyPoolsV2::POLY_INPUT));
    ...
    addLabeledKnob("Num Output Channels", 10, 156, module,
                   ComputerscareTolyPoolsV2::NUM_CHANNELS_KNOB, -14, -24, 0);
    addInput(createInput<InPort>(Vec(10, 186), module,
                                 ComputerscareTolyPoolsV2::NUM_CHANNELS_CV));

    addLabeledKnob("Rotation", 10, 256, module,
                   ComputerscareTolyPoolsV2::ROTATE_KNOB, -13, -5, 1);
    addInput(createInput<InPort>(Vec(10, 286), module,
                                 ComputerscareTolyPoolsV2::ROTATE_CV));

    addOutput(createOutput<OutPort>(Vec(28, 30), module,
                                    ComputerscareTolyPoolsV2::POLY_OUTPUT));

    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(31, 76), module, ComputerscareTolyPoolsV2::NUM_CHANNELS_OUTPUT));
  }
  void addLabeledKnob(std::string label, int x, int y,
                      ComputerscareTolyPoolsV2* module, int index,
                      float labelDx, float labelDy, int type) {
...
void ComputerscareTolyPoolsWidgetV2::appendContextMenu(Menu* menu) {
  ...
  menu->addChild(construct<PoolsModeItem>(
      &MenuItem::text, "Repeat Input Channels", &PoolsModeItem::pools, pools,
      &PoolsModeItem::modeEnum, 0));
  menu->addChild(construct<PoolsModeItem>(
      &MenuItem::text, "Rotate Through Maximum of Output, Input Channels",
      &PoolsModeItem::pools, pools, &PoolsModeItem::modeEnum, 1));
  menu->addChild(construct<PoolsModeItem>(
      &MenuItem::text, "Rotate Through 16 Channels (Legacy)",
      &PoolsModeItem::pools, pools, &PoolsModeItem::modeEnum, 2));
}

Model* modelComputerscareTolyPoolsV2 =
    createModel<ComputerscareTolyPoolsV2, ComputerscareTolyPoolsWidgetV2>(
