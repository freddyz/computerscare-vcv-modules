...
struct ComputerscareRolyPouter : ComputerscarePolyModule {
  ...
  enum InputIds { POLY_INPUT, ROUTING_CV, NUM_INPUTS };
  ...
  ComputerscareRolyPouter() {
    ...
    for (int i = 0; i < numKnobs; i++) {
      configParam(KNOB + i, 1.f, 16.f, (i + 1),
  ...
  void process(const ProcessArgs& args) override {
    ...
    if (cvChannels > 0) {
      for (int i = 0; i < numOutputChannels; i++) {
        knobSetting =
            std::round(inputs[ROUTING_CV].getVoltage(cvChannels == 1 ? 0 : i) *
                       1.5) +
            1;
        ...
        if (routing[i] > inputChannels) {
          ...
          outputs[POLY_OUTPUT].setVoltage(
              inputs[POLY_INPUT].getVoltage(routing[i]), i);
        }
      }
    } else {
      if (counter > 8) {
      ...
      for (int i = 0; i < numOutputChannels; i++) {
        ...
        if (knobSetting > inputChannels) {
          ...
          outputs[POLY_OUTPUT].setVoltage(
              inputs[POLY_INPUT].getVoltage(knobSetting - 1), i);
        }
      }
    }
  }
...
struct PouterSmallDisplay : SmallLetterDisplay {
  ...
  void draw(const DrawArgs& args) {
    if (module) {
      ...
      if (module->numInputChannels > 0 &&
          (module->routing[ch] > module->numInputChannels)) {
...
struct ComputerscareRolyPouterWidget : ModuleWidget {
  ComputerscareRolyPouterWidget(ComputerscareRolyPouter* module) {
    ...
    channelWidget = new PolyOutputChannelsWidget(
        Vec(2, 8), module, ComputerscareRolyPouter::POLY_CHANNELS);
    addChild(channelWidget);

    addInput(createInput<PointingUpPentagonPort>(
        Vec(22, 52), module, ComputerscareRolyPouter::ROUTING_CV));
    ...
    addInput(createInput<InPort>(Vec(1, 36), module,
                                 ComputerscareRolyPouter::POLY_INPUT));
    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(32, 18), module, ComputerscareRolyPouter::POLY_OUTPUT));
  }
  void addLabeledKnob(std::string label, int x, int y,
                      ComputerscareRolyPouter* module, int index, float labelDx,
                      float labelDy) {
    ...
    DisableableSnapKnob* knob = createParam<DisableableSnapKnob>(
        Vec(x, y), module, ComputerscareRolyPouter::KNOB + index);
    knob->channel = index;
    knob->module = module;
    addParam(knob);
  ...
  void appendContextMenu(Menu* menu) override {
    ...
    struct OneToOneItem : MenuItem {
      ...
      void step() override {
        rightText =
            pouter->params[ComputerscareRolyPouter::RANDOMIZE_ONE_TO_ONE]
                        .getValue() == 1.f
                ? "✔"
                : "";
        MenuItem::step();
...
Model* modelComputerscareRolyPouter =
    createModel<ComputerscareRolyPouter, ComputerscareRolyPouterWidget>(
