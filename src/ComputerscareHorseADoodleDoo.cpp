...
const std::string HorseAvailableModes[4] = {
    ...
    "Trigger Cascade:\nEach channel is triggered by the previous channel's "
    "trigger sequence",
...
struct HorseSequencer {
  ...
  int primes[16] = {30011, 36877, 26627, 32833, 66797, 95153, 66553, 84857,
                    32377, 79589, 25609, 20113, 70991, 86533, 21499, 32491};
  int otherPrimes[16] = {80651, 85237, 11813, 22343, 19543, 28027,
                         9203,  39521, 42853, 58411, 33811, 76771,
  ...
  void makeAbsolute() {
    ...
    for (int i = 0; i < numSteps; i++) {
      ...
      for (int k = 0; k < 4; k++) {
        ...
        cvVal += std::sin(primes[cvArgIndex] * arg + otherPrimes[cvThetaIndex] +
                          phase);

        cv2Val += std::sin(primes[cv2ArgIndex] * arg +
                           otherPrimes[cv2ThetaIndex] + phase2);
  ...
  void checkAndArm(float patt, int steps, float dens, float phi, float phi2,
                   float gatePhi) {
    if (pattern != patt || numSteps != steps || density != dens ||
        phase != phi || phase2 != phi2 || gatePhase != gatePhi) {
      pendingPattern = patt;
  ...
  void armChange() { forceChange = true; }
  void disarm() {
    pendingChange = false;
  ...
  void tick() {
    ...
    if ((currentStep == 0 && pendingChange) || forceChange) {
      change(pendingPattern, pendingNumSteps, pendingDensity, pendingPhase,
             pendingPhase2, pendingGatePhase);
      pendingChange = false;
      forceChange = false;
      currentStep = 0;
    }
  ...
  int get() { return absoluteSequence[currentStep]; }
  float getCV() { return cvSequence[currentStep]; }
  float getCV2() { return cv2Sequence[currentStep]; }
  float getGateLength() { return gateLengthSequence[currentStep]; }
  int getTimeToNextStep() { return timeToNextStep[currentStep]; }
  int tickAndGet() {
    tick();
    return get();
  }
...
struct ComputerscareHorseADoodleDoo : ComputerscareMenuParamModule {
  ...
  float cv2Val[16] = {0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f,
                      0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f, 0.f};
  ...
  float gateTimeFactor[16] = {.999f, .999f, .999f, .999f, .999f, .999f,
                              .999f, .999f, .999f, .999f, .999f, .999f,
                              .999f, .999f, .999f, .999f};

  float syncTime[16] = {1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f,
                        1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f, 1.f};
  HorseSequencer seq[16];

  struct HorsePatternParamQ : ParamQuantity {
    virtual std::string getPatternString() {
      return dynamic_cast<ComputerscareHorseADoodleDoo*>(module)
          ->getPatternDisplay();
    }
    std::string getDisplayValueString() override {
      float val = getValue();
  ...
  ComputerscareHorseADoodleDoo() {
    ...
    configParam<HorsePatternSpreadParam>(PATTERN_SPREAD, 0.f, 1.f, 0.5f,
                                         "Pattern Spread", "", 0, 100);
    configParam<HorseStepsSpreadParam>(STEPS_SPREAD, -1.f, 1.f, 0.f,
                                       "Steps Spread", "", 0, 100);
    ...
    configMenuParam(GATE_LENGTH_OFFSET, 0.f, 1.f, 0.f, "Gate Length Minimum",
                    2);
    configMenuParam(GATE_LENGTH_PHASE, -3.14159f, 3.14159f, 0.f,
                    "Gate Length Variation", 2);
  ...
  std::string getPatternDisplay(bool showPatternValue = false,
    ...
    for (int i = 0; i < polyChannels; i++) {
      ...
      for (int j = 0; j < seq[channelToDisplay].numSteps; j++) {
        ...
        out += (showTransport && current == j) ? (highStep ? "☺" : "☹")
                                               : (highStep ? "x" : "_");
        out += j % 192 == 191 ? "\n" : "";
      }
  ...
  void checkKnobChanges() {
    ...
    polyChannels =
        lastPolyKnob == 0
            ? std::max(clockNum,
                       std::max(pattNum, std::max(stepsNum, densityNum)))
            : lastPolyKnob;
    ...
    for (int i = 0; i < polyChannels; i++) {
      float patternVal = params[PATTERN_KNOB].getValue() +
                         params[PATTERN_TRIM].getValue() *
                             inputs[PATTERN_CV].getVoltage(
                                 pattNum == 1 ? 0 : fmin(i, pattNum));
      int stepsVal = std::floor(params[STEPS_KNOB].getValue() +
                                params[STEPS_TRIM].getValue() *
                                    inputs[STEPS_CV].getVoltage(
                                        stepsNum == 1 ? 0 : fmin(i, stepsNum)));
      float densityVal = params[DENSITY_KNOB].getValue() +
                         params[DENSITY_TRIM].getValue() *
                             inputs[DENSITY_CV].getVoltage(
                                 densityNum == 1 ? 0 : fmin(i, densityNum)) /
                             10;
      ...
      seq[i].checkAndArm(patternVal, stepsVal, densityVal, lastPhaseKnob,
                         lastPhase2Knob, lastGatePhaseKnob);
    }
  }
  void processChannel(int ch, bool clocked, bool reset, bool clockInputHigh,
    ...
    if (clocked /*&& !reset*/) {
      if (overrideMode == 1) {
        ...
        // no override, operate as normal.  Tick sequencer every clock, and tick
        // CV if the trigger is high
        seqVal[ch] = seq[ch].tickAndGet();
        if (seqVal[ch]) {
          cvVal[ch] = seq[ch].getCV();
          cv2Val[ch] = seq[ch].getCV2();
      ...
      shouldOutputPulse[ch] =
          seqVal[ch] == 1 && (previousStep[ch] != seq[ch].currentStep);
      ...
      if (gateMode == 1 && shouldOutputPulse[ch]) {
        ...
        float gateLengthFactor =
            math::clamp(gateLengthOffset + gateLengthScale * ((float)len) / 16,
                        0.05f, 0.95f);
        float ms = timeLeft * gateLengthFactor;
        if (ch == 0 || ch == 1) {
          // DEBUG("ch:%i,step:%i,len:%i,ttns:%i,ms:%f", ch,
          // seq[ch].currentStep, len, ttns, ms);
        }
    ...
    if (true || inputs[CLOCK_INPUT].isConnected()) {
      if (gateMode == 0) {
        ...
        outputs[TRIGGER_OUTPUT].setVoltage(
            (clockInputHigh && shouldOutputPulse[ch]) ? 10.0f : 0.0f, ch);
      ...
      // outputs[EOC_OUTPUT].setVoltage((currentTriggerIsHigh &&
      // atFirstStepPoly[ch]) ? 10.f : 0.0f, ch);
    } else {
    }

    // if (atFirstStepPoly[ch]) {
    outputs[EOC_OUTPUT].setVoltage(
        (clockInputHigh && shouldSetEOCHigh[ch]) ? 10.f : 0.0f, ch);
    //}
  }
  ...
  void process(const ProcessArgs& args) override {
    ...
    bool manualReset = globalManualResetTrigger.process(
        params[MANUAL_RESET_BUTTON].getValue());
    bool manualClock =
        clockManualTrigger.process(params[MANUAL_CLOCK_BUTTON].getValue());
    ...
    if (mode == 0) {
      ...
      for (int i = 0; i < 16; i++) {
        if (i == 0) {
          ...
          processChannel(i, currentClock[clockChannels[i] - 1],
                         currentReset[resetChannels[i] - 1],
                         isHigh[clockChannels[i] - 1], mode, seqVal[i - 1]);
        }
      }
    } else if (mode == 3) {
      ...
      for (int i = 0; i < 16; i++) {
        if (i == 0) {
          ...
          processChannel(i, currentClock[clockChannels[i] - 1],
                         currentReset[resetChannels[i] - 1],
                         isHigh[clockChannels[i] - 1], mode,
                         shouldSetEOCHigh[i - 1]);
        }
      }
    }
  }
  ...
  void draw(const DrawArgs& args) {
    if (module) {
      ...
      if (knobConnection == 1) {
        ...
        str = module->lastPolyKnob == 0 ? "A"
                                        : std::to_string(module->lastPolyKnob);
      }
      value = str;
    } else {
...
struct setGateModeItem : MenuItem {
  ...
  void step() override {
    rightText = CHECKMARK(
        horse->params[ComputerscareHorseADoodleDoo::GATE_MODE].getValue() ==
        mySetVal);
    MenuItem::step();
  }
};
...
struct ModeChildMenu : MenuItem {
  ...
  Menu* createChildMenu() override {
    ...
    menu->addChild(construct<MenuLabel>(
        &MenuLabel::text, "How the polyphonic channels are triggered"));

    for (unsigned int i = 0; i < 4; i++) {
      setModeItem* menuItem = new setModeItem(i);
      // ParamSettingItem *menuItem = new
      // ParamSettingItem(i,ComputerscareGolyPenerator::ALGORITHM);
...
struct ComputerscareHorseADoodleDooWidget : ModuleWidget {
  ComputerscareHorseADoodleDooWidget(ComputerscareHorseADoodleDoo* module) {
    ...
    addInputBlock("Pattern", 10, 100, module, 0,
                  ComputerscareHorseADoodleDoo::PATTERN_CV, 0,
                  ComputerscareHorseADoodleDoo::PATTERN_SPREAD);
    addInputBlock("Length", 10, 150, module, 2,
                  ComputerscareHorseADoodleDoo::STEPS_CV, 1,
                  ComputerscareHorseADoodleDoo::STEPS_SPREAD, false);
    ...
    addParam(createParam<ComputerscareClockButton>(
        Vec(2, inputY - 6), module,
        ComputerscareHorseADoodleDoo::MANUAL_CLOCK_BUTTON));
    addInput(createInput<InPort>(Vec(2, inputY + 10), module,
                                 ComputerscareHorseADoodleDoo::CLOCK_INPUT));

    addParam(createParam<ComputerscareResetButton>(
        Vec(2, inputY + dy + 16), module,
        ComputerscareHorseADoodleDoo::MANUAL_RESET_BUTTON));

    addInput(createInput<InPort>(Vec(2, inputY + 2 * dy), module,
                                 ComputerscareHorseADoodleDoo::RESET_INPUT));

    channelWidget =
        new PolyOutputChannelsWidget(Vec(outputX + 18, inputY - 22), module,
                                     ComputerscareHorseADoodleDoo::POLY_KNOB);
    addChild(channelWidget);

    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(outputX, outputY), module,
        ComputerscareHorseADoodleDoo::TRIGGER_OUTPUT));
    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(outputX, outputY + dy), module,
        ComputerscareHorseADoodleDoo::EOC_OUTPUT));
    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(outputX, outputY + dy * 2), module,
        ComputerscareHorseADoodleDoo::CV_OUTPUT));
    addOutput(createOutput<PointingUpPentagonPort>(
        Vec(outputX, outputY + dy * 3), module,
        ComputerscareHorseADoodleDoo::CV2_OUTPUT));
  }

  void addInputBlock(std::string label, int x, int y,
                     ComputerscareHorseADoodleDoo* module, int knobIndex,
                     int inputIndex, int knobType, int scrambleIndex,
                     bool allowScrambleRandom = true) {
    ...
    if (knobType == 0) {  // smooth
      ...
      if (knobType == 1) {
        // trim knob
        // addParam(createParam<SmallKnob>(Vec(x + 30, y), module, knobIndex +
        // 1)); addInput(createInput<TinyJack>(Vec(x + 40, y), module,
        // inputIndex)); addParam(createParam<ScrambleKnob>(Vec(x+30, y+20),
        // module, scrambleIndex));
      }
    }
    ...
    if (allowScrambleRandom) {
      addParam(createParam<ScrambleKnob>(Vec(x + 55, y - 15), module,
                                         scrambleIndex));
    } else {
      addParam(createParam<ScrambleKnobNoRandom>(Vec(x + 55, y - 15), module,
                                                 scrambleIndex));
    }
  }
...
Model* modelComputerscareHorseADoodleDoo =
    createModel<ComputerscareHorseADoodleDoo,
