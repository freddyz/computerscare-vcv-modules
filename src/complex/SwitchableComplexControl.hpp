...
struct SwitchableComplexControl : Widget {
	...
	SwitchableComplexControl(ComputerscareComplexBase* module, int paramIndex,
	                         ...
	                         ComplexXYMaxMode arrowMaxMode,
	                         float arrowMaxVoltage, int laneIndex = -1,
	                         bool showDisabledOverlay = false) {
		this->module = module;
	...
	void step() override {
		...
		if (currentMode != lastMode) {
			if (lastMode == 2) {
				ComplexControl::syncPolarParamsToRectParams(
					module, polarParamIndex, paramIndex);
			}
			if (currentMode == 2) {
				ComplexControl::syncRectParamsToPolarParams(
